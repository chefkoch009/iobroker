...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'K.Ringmann';
    var email     = grunt.option('email')  || 'info@punktnetzwerk.net';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new eib-knx name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/knx.png')) {
            fs.renameSync(__dirname + '/admin/knx.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/knx.html')) {
            fs.renameSync(__dirname + '/widgets/knx.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/knx/js/knx.js')) {
            fs.renameSync(__dirname + '/widgets/knx/js/knx.js', __dirname + '/widgets/knx/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/knx')) {
            fs.renameSync(__dirname + '/widgets/knx',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /knx/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'knx'
                        },
                        {
                            match: /K.Ringmann/g,
                            replacement: author
                        ...
                        {
                            match: /info@punktnetzwerk.net/g,
                            replacement: email
                        }
                    ]
                },
